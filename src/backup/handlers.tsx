...
export function backupHandlers(db: Database, config: Config) {
  return new Elysia({ name: "backup-handlers" })
    ...
    .post("/backup/restore/upload", async ({ body, session }) => {
      ...
      const encodedData = Buffer.from(JSON.stringify(parsed.value)).toString(
        "base64",
      );
    ...
    .post("/backup/restore/confirm", ({ body, session, set }) => {
...
export function backupApiHandlers(db: Database) {
  return new Elysia({ name: "backup-api-handlers" }).get("/backup", () => {

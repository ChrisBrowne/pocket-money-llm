...
The app is server-rendered with HTMX. Errors need to reach the user as HTML. There are two fundamentally different kinds of failure:
...
Every page layout includes a generic error element (e.g. a toast or banner container). Elysia's top-level error handler catches unhandled exceptions and returns an HTMX out-of-band (OOB) swap targeting this element with a generic error message.

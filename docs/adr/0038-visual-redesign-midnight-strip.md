...
- **Accessibility maintained**: every `prefers-reduced-motion: reduce` viewer gets the flicker and pulse animations disabled. The sun-grid floor was originally animated too, but the scroll animation was dropped in a follow-up — laptop fans spun up under it, and the static grid reads visually identical. Focus rings are explicit on inputs (neon glow). Color contrast on body text (`var(--color-ink)` on `var(--color-bg)`) meets WCAG AA.

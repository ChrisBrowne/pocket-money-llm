...
- **Caching.** Both jobs cache `node_modules` keyed on `bun.lock` so unchanged dependencies become a no-op install. The `e2e` job additionally caches `~/.cache/ms-playwright` (the chromium binary) keyed on `bun.lock`. Neither cache uses `restore-keys`: a partial-hit on `node_modules` would invite version drift, and an older chromium against a newer `@playwright/test` would produce mysterious test failures — exact match or fresh install is the safer default.

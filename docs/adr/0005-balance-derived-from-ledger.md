...
A child's balance could be stored as a field updated on each transaction, or derived by summing the transaction ledger. Stored balances are faster to read but can drift from reality. Derived balances are always correct but require computation.
